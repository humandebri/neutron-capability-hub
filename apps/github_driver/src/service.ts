...
exposeTool("github_repositories_list", {
  ...
  return withGitHub(async (api) => {
    ...
    if (consumer === "github_driver") return { repositories: repositories.map(publicRepository) };
...
exposeTool("github_issues_list", {
  ...
  if (consumer !== "github_driver" && !(await authorizePolicy(consumer, owner, repo, "issue_read"))) throw new Error("GitHub Driver Policy denied issue_read for this consumer and repository");
